#!/usr/bin/env python3
"""
MVtext.bin 텍스트 추출/임포트 도구
Xenosaga Episode III 무비 뷰어 텍스트 파일

파일 구조:
  [0x00~0x2f] 파일 헤더 (48바이트):
    [0~3]   데이터셋 개수 (uint32 LE)
    [4~...]  (offset LE, size LE) 쌍 × 개수
    나머지:  0x90 패딩
  데이터셋 0~2: MVD 헤더 반복 (각 48바이트) + 문자열 영역
    [0~3]   'MVD\x00' 태그
    [4~7]   인덱스1
    [8~11]  인덱스2
    [12~15] p1 (ds_off 기준 상대 오프셋)
    [16~19] p2 (ds_off 기준 상대 오프셋)
    [20~23] ID/크기값
    [24~27] 복합 필드
    [28~35] 0
    [36~47] 0x90 패딩
  데이터셋 3: 포인터 배열 + 문자열 (njDBcommon 방식)
...
def get_datasets(data):
    ...
    for i in range(num):
        ...
        is_mvd = data[offset:offset+4] == b'MVD\x00'
        result.append({'index': i, 'offset': offset, 'size': size, 'is_mvd': is_mvd})
    return result
...
def collect_strings(data):
    """
    포인터 기반으로 모든 문자열 수집.
    - MVD 데이터셋: p1/p2 (ds_off 기준 상대 오프셋)
    - 포인터 배열 데이터셋: 상대 오프셋 배열
    abs_pos 기준 중복 제거.
    """
    ...
    for ds in datasets:
        ...
        if ds['is_mvd']:
            pos       = ds_off
            entry_idx = 0
            while pos + MVD_SIZE <= ds_off + ds_size:
                if data[pos:pos+4] != b'MVD\x00':
                    break
                p1 = struct.unpack_from('<I', data, pos + 12)[0]
                p2 = struct.unpack_from('<I', data, pos + 16)[0]
                for field, ptr in (('p1', p1), ('p2', p2)):
...
def patch_ptrs(data, ds_info, old_ptr, new_ptr):
    """ds_off 기준 상대 포인터 old_ptr → new_ptr 교체."""
...
def main():
    if len(sys.argv) < 3:
        print("사용법:\n"
              "  python mvtexttool.py extract <bin> [출력.txt] [치환표.json]\n"
              "  python mvtexttool.py import  <bin> <입력.txt> [출력.bin] [치환표.json]")
        sys.exit(1)
